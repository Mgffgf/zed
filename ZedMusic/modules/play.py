...
from ZedMusic.config import ARQ_API_KEY
...
from ZedMusic.function.admins import admins as a
from ZedMusic.helpers.admins import get_administrators
from ZedMusic.helpers.channelmusic import get_chat_id
from ZedMusic.helpers.decorators import authorized_users_only
from ZedMusic.helpers.filters import command, other_filters
...
@Client.on_message(filters.command("اغاني") & filters.group & ~filters.edited)
async def playlist(client, message):
    ...
    if not queue:
        await message.reply_text("**•🝰╎لاتوجـد اغـانـي في القائمـة** !")
    temp = []
    for t in queue:
        temp.append(t)
...
def r_ply(type_):
    ...
    mar = InlineKeyboardMarkup(
        [
            ...
            [InlineKeyboardButton("❌ اغـلاق", "cls")],
        ]
    )
    return mar


@Client.on_message(filters.command("مسار") & filters.group & ~filters.edited)
async def ee(client, message):
    ...
    if stats:
        ...
        await message.reply("**•🝰╎لاتوجـد اغـانـي مشـغلـه هنـا** !")


@Client.on_message(filters.command("تحكم") & filters.group & ~filters.edited)
...
@Client.on_callback_query(filters.regex(pattern=r"^(قائمه)$"))
async def p_cb(b, cb):
    global que
        ...
        for administrator in administrators:
            if administrator == message_.from_user.id:
                ...
                try:
                    ...
                    await lel.edit(
                        f"<b>🔴 Flood Wait Error 🔴 \nUser {user.first_name} couldn't join your group due to heavy requests for userbot! Make sure user is not banned in group."
                        "\n\nOr manually add @ZedMusic to your Group and try again</b>",
                    )
    try:
        await USER.get_chat(chid)
        # lmoa = await client.get_chat_member(chid,wew)
