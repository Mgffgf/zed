...
from ZedMusic.config import BOT_NAME as bn
from ZedMusic.config import DURATION_LIMIT
from ZedMusic.config import UPDATES_CHANNEL as updateschannel
from ZedMusic.config import que
from ZedMusic.helpers.admins import get_administrators
from ZedMusic.helpers.decorators import authorized_users_only
from ZedMusic.helpers.gets import get_file_name
from ZedMusic.modules.play import arq, cb_admin_check, generate_cover
...
@Client.on_message(
    filters.command(["الاغاني", "اغانيي"]) & filters.group & ~filters.edited
)
async def playlist(client, message):
    try:
        ...
        message.reply("**␥┆هل هـذه القنـاة مرتبطـة بالحسـاب...**")
        return
    global que
    queue = que.get(lol)
    if not queue:
...
def r_ply(type_):
    ...
    mar = InlineKeyboardMarkup(
        [
            ...
            [InlineKeyboardButton("❌ اغـلاق", "ccls")],
        ]
    )
    return mar
...
@Client.on_message(
    filters.command(["المسار", "مسارر"]) & filters.group & ~filters.edited
)
...
@Client.on_callback_query(filters.regex(pattern=r"^(القائمه)$"))
async def p_cb(b, cb):
    global que
    try:
...
@Client.on_message(
    filters.command(["التشغيل", "تشغيلل"]) & filters.group & ~filters.edited
)
@authorized_users_only
async def play(_, message: Message):
    global que
    lel = await message.reply("🔄 <b>جـاري التشغيـل</b>")
    ...
    try:
        ...
        return
    text_links = None
    await lel.edit("🔎 <b>نتائـج البحث</b>")
    ...
    if audio:
        ...
        keyboard = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton("📖 قائمـة التشغيل", callback_data="cplaylist"),
                    InlineKeyboardButton("القائمـه ⏯ ", callback_data="cmenu"),
                ],
                [InlineKeyboardButton(text="❌ اغـلاق", callback_data="ccls")],
            ]
        )
        file_name = get_file_name(audio)
        title = file_name
        thumb_name = "https://telegra.ph/file/72e136a1c7f27cb5e9399.png"
        ...
        keyboard = InlineKeyboardMarkup(
            [
                ...
                [
                    InlineKeyboardButton(text="🎬 يوتيـوب", url=f"{url}"),
                    InlineKeyboardButton(text="تحميـل 📥", url=f"{dlurl}"),
                ],
                [InlineKeyboardButton(text="❌ اغـلاق", callback_data="ccls")],
            ]
        )
...
@Client.on_message(
    filters.command(["التحكم", "تحكمم"]) & filters.group & ~filters.edited
)
@authorized_users_only
async def jiosaavn(client: Client, message_: Message):
    global que
    lel = await message_.reply("🔄 **جـاري...**")
    try:
        ...
        await message_.reply("**␥┆هل هـذه القنـاة مرتبطـة بالحسـاب...**")
        return
    try:
        administrators = await get_administrators(conv)
    except:
    ...
    query = text[1]
    res = lel
    await res.edit(f"جـاري البحث 🔎 عن `{query}` على jio saavn")
    try:
        ...
        sthumb = "https://telegra.ph/file/72e136a1c7f27cb5e9399.png"
        sduration = int(songs.result[0].duration)
    except Exception as e:
        await res.edit("لم يتم العثور على أي شيء حرفيًا!، يجب أن تبحث بـ اللغة الإنجليزية وبصورة صحيحة.")
        print(str(e))
        return
    keyboard = InlineKeyboardMarkup(
        [
            ...
            [
                InlineKeyboardButton(
                    text="• قنـاة التحديثـات •", url=f"https://t.me/{updateschannel}"
                )
            ],
            [InlineKeyboardButton(text="❌ اغـلاق", callback_data="ccls")],
        ]
    ...
    if chat_id in callsmusic.active_chats:
        ...
        m = await client.send_photo(
            ...
            reply_markup=keyboard,
            photo="final.png",
            caption=f"✯{bn}✯=#️⃣ في قائمـة الانتظـار عنـد {position}",
        )

    else:
        await res.edit_text(f"{bn}=▶️ يتـم التشغيـل.....")
        que[chat_id] = []
        qeue = que.get(chat_id)
        s_name = sname
        r_by = message_.from_user
    ...
    m = await client.send_photo(
        ...
        reply_markup=keyboard,
        photo="final.png",
        caption=f"يتـم تشغيـل {sname} عبر Jiosaavn في القنـاة",
    )
    os.remove("final.png")
